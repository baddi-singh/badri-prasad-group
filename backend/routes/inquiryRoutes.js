...
const sendEmail = require('../utils/sendEmail'); // 🔥 Centralized Utility import ki
const { protect } = require('../middleware/authMiddleware');

// 1. GET: Admin Dashboard ke liye saari inquiries laana
...
// 2. POST: Public Form Submission (Database Save + Dual Email Dispatch)
router.post('/', async (req, res) => {
  try {
    ...
    const adminRecipient = process.env.ADMIN_EMAIL || process.env.EMAIL_USER || 'contact@badriprasadgroup.com';
    ...
    // B. Client Confirmation Auto-Reply
    const clientHtml = `
    ...
    // Emails dispatch (Non-blocking)
    try {
      await sendEmail({
        to: adminRecipient,
        subject: `⚡ New Executive Inquiry: ${entityName} - ${senderName}`,
        html: adminHtml,
        from: `Badri Prasad Group Portal <contact@badriprasadgroup.com>`
      });
      ...
      if (senderEmail) {
        await sendEmail({
          ...
          html: clientHtml,
          from: `Badri Prasad Group <contact@badriprasadgroup.com>`
        });
      }
    } catch (mailError) {
