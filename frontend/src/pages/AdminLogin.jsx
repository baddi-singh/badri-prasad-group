...
// Running code hai, backend ke saath properly kaam kar raha hai.
// import React, { useState } from 'react';
// import axios from 'axios';
// import { useNavigate } from 'react-router-dom';
...
//     try {
//       // 🔥 DEVIL FIX: Backend 'email' mang raha tha, par tum 'userId' bhej rahe the. Fix kar diya.
//       const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/admin/login`, {
//         email: email, 
//         password: password
//       }, {
//         // 🔥 DEVIL FIX: Backend se aayi cookie browser ko save karne ki permission dena
//         withCredentials: true 
//       });

//       if (res.data.success) {
//         // Sirf ek UI flag, asli security cookie ke paas hai
...
//   return (
//     <section className="container animate-up" style={{ 
//       minHeight: '100vh', background: '#050505', paddingTop: '120px', 
//       paddingBottom: '100px', display: 'flex', alignItems: 'center', justifyContent: 'center' 
//     }}>
//       <div style={{ 
//         maxWidth: '450px', width: '100%', padding: '45px 40px', 
//         background: 'rgba(15, 15, 15, 0.8)', backdropFilter: 'blur(12px)',
...
//               style={{ width: '100%', padding: '14px 15px', background: '#0a0a0a', border: '1px solid #333', borderRadius: '8px', color: '#fff', fontSize: '14px', outline: 'none', transition: 'border 0.3s ease', boxSizing: 'border-box' }}
//               onFocus={(e) => e.target.style.borderColor = '#D4AF37'} onBlur={(e) => e.target.style.borderColor = '#333'} required 
//             />
//           </div>
...
//             style={{ width: '100%', padding: '15px', background: '#D4AF37', color: '#000', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '900', letterSpacing: '1.5px', cursor: 'pointer', transition: 'all 0.3s ease', boxSizing: 'border-box' }}
//             onMouseOver={(e) => { if(!loading){ e.target.style.background = '#e6c245'; e.target.style.transform = 'translateY(-2px)'; e.target.style.boxShadow = '0 5px 15px rgba(212,175,55,0.4)'; } }}
//             onMouseOut={(e) => { if(!loading){ e.target.style.background = '#D4AF37'; e.target.style.transform = 'translateY(0)'; e.target.style.boxShadow = 'none'; } }}
//           >
//             {loading ? 'AUTHENTICATING...' : 'SECURE LOGIN'}
//           </button>
//         </form>
...
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import api from '../api/axios'; // 🔥 Naya Axios instance import kiya
...
const AdminLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // 🔥 DEVIL FIX: api instance use kiya, aur withCredentials: true lagaya
      ...
      if (res.data.success) {
        // Agar backend token bhej raha hai toh save karo
        if (res.data.token) {
